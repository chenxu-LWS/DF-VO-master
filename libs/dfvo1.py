...
class DFVO():
    def __init__(self, cfg):
        ...
        # predicted gloabl poses
        self.gloabl_poses = {0:SE3()}
    ...
    def update_global_pose(self, new_pose, scale=1.):
        ...
        self.gloabl_poses[self.cur_data['id']] = copy.deepcopy(self.cur_data['pose'])
    ...
    def tracking(self):
        ...
        # Second to last frames
            ...
            ''' Pnp-tracker '''
            if self.cfg.tracking_method in ['Pnp', 'hybrid']:
    ...
    def deep_model_inference(self):
        ...
        if self.tracking_stage >= 1 and self.cfg.deep_pose.enable:
            ...
            pose = self.deep_models.forward_pose(
                        [self.ref_data['image'], self.cur_data['image']]
                        )
        ...
        # Matrix Fisher
        if self.tracking_stage >=1:
            self.timers.start('Fisher_net')
            params_out = self.deep_models.forward_fisher(
                        [self.ref_data['img'], self.cur_data['img']]
            )
            self.ref_data['rotation'] = params_out  # from cur->ref
            self.timers.end('Fisher_net')
